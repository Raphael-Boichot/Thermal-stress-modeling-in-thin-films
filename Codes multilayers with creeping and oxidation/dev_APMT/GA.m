...
borne = [0.05 0.001 10];    % intervalle de recherche x,y=[-val val]
...
if isfile('sol.tmp')
    ...
    [new_sol]=fminsearch(objectif,previous_sol,options);
    [new_sol]=fminsearch(objectif,new_sol,options);%because
    [new_sol,fval]=fminsearch(objectif,new_sol,options);%because
...
if skip==0
    ...
    for i=1:1:10000
        for j=1:1:nb_var
            pop_ini(i,j)=borne(1,j)*rand-2*borne(1,j)*rand;
        end
        vecteur_initial(i,1)=objectif(pop_ini(i,:));
    end
    % min(vecteur_initial)
    % max(vecteur_initial)
    % hist(vecteur_initial,30)
    % drawnow
    ...
    while(tol<obj)
        ...
        for i=1:nb_pop
            ...
            fitness_temp=objectif(pop(i,:));
            [pop_temp,fitness]=fminsearch(objectif,pop(i,:),options);
            if fitness<fitness_temp;
            pop(i,:)=pop_temp;
        ...
        for i=3:2:nb_pop
            ...
            popnew=croisement_GA(i,i1,i2,pop,popnew,p_croisement,p_mutation,0.1);
        end
        pop=popnew;
