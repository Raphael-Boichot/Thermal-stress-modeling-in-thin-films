...
T_depot=1200;%donc on est en refroissement ici
t_s=470e-6; %m, épaisseur du substrat
t_f=1e-6; %m, épaisseur du film
eps_misfit=-0.7e-2; %deformation de misfit
L_ilots=30e-9; %effet des ilots de nucléation
ilots=1; % =pour activer l'effet des ilots
...
fval=0;
while fval==0;
[x,fval]=GA(@contrainte_multicouche_Hsueh_BOICHOT,3,1e-8);
end;

disp('Réussite de la méthode et convergence');
c=x(1);%deformation finale
t_b=x(2);%position fibre neutre
r=x(3);%rayon de courbure
